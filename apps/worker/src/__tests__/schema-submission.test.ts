...
import {
  submitSchemaAudit,
  __resetSchemaSubmissionQueueForTests,
...
describe("submitSchemaAudit", () => {
  ...
  it("inserts the job record and returns its id when the enqueue succeeds", async () => {
    ...
    const result = await submitSchemaAudit({
      db,
      redisUrl: REDIS_URL,
      url: TARGET_URL,
    });
  ...
  it("deletes the orphaned job record and surfaces the error when the enqueue fails", async () => {
    ...
    await expect(
      submitSchemaAudit({ db, redisUrl: REDIS_URL, url: TARGET_URL }),
