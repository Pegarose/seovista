...
import {
  ...
  __resetRenderParitySubmissionQueueForTests,
...
describe("submitRenderParityCheck", () => {
  beforeEach(() => {
  ...
  it("inserts exactly one job_records row with the render_parity_audit queue name and returns its id", async () => {
    submitMock.add.mockResolvedValue({});
    const { db, calls } = createFakeDb();

    const result = await submitRenderParityCheck(buildInput(db));

    const inserts = calls.filter((c) => /INSERT INTO job_records/.test(c.sql));
    expect(inserts).toHaveLength(1);
    const insert = inserts[0]!;
    expect(insert.params![2]).toBe("render_parity_audit");
  ...
  it("deletes the orphaned job record and surfaces the error when the enqueue fails", async () => {
    ...
    await expect(submitRenderParityCheck(buildInput(db))).rejects.toBe(enqueueError);
  ...
  it("exposes a callable close-queue helper", async () => {
    await expect(closeRenderParitySubmissionQueue()).resolves.toBeUndefined();
  });
});
