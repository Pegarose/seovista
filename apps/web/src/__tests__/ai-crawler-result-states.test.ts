/**
 * AI Crawler Result Page State Contract Tests (B4)
 *
 * The ai-crawler-checker result page at
 * `/tools/ai-crawler-checker/result/[jobId]` must render an explicit
 * unknown-status state for any persisted job_records.status outside the
 * supported lifecycle vocabulary instead of crashing on the result payload,
 * while preserving the one-main/one-h1 landmark contract. Follows the
 * geo-result-states.test.ts conventions.
 */
...
describe("AI Crawler Result Page unknown status guard (B4)", () => {
  ...
  it("completed status with valid payload still renders the result (guard does not break the happy path)", async () => {
    ...
    expect(countTag(text, "main")).toBe(1);
    expect(countTag(text, "h1")).toBe(1);
    expect(text).toContain("AI Crawler Erişim Denetim Sonucu");
