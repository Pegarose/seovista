...
/**
 * Form-level input validation for the Schema Checker.
 *
 * This module intentionally has NO "use server" directive: Next.js rejects
 * non-async exports from server-action files, and `validateSchemaInput` is a
 * synchronous helper shared between the server action and unit tests.
 *
 * SSRF note: this is a defense-in-depth form guard only. The authoritative
 * SSRF boundary is the worker fetcher (`apps/worker/src/utils/fetcher.ts`),
 * which performs DNS resolution and blocks loopback/link-local/private/
 * reserved ranges via ipaddr.js before any network call.
 */
