...
async function main() {
  ...
  // Derive DATABASE_URL for migrations
  const databaseUrl = `postgres://seovista:seovista@127.0.0.1:${dbPort}/${dbName}`;
  
  process.env.DATABASE_URL = databaseUrl;
  process.env.SEOVISTA_ADMIN_PASSWORD = "test-admin-password"; // Ensure admin-bootstrap runs
  
  // Use Corepack pnpm to run db:bootstrap from worker where it exists!
