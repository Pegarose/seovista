9c1b6a5 fix(worker,web): apply final review fixes for B3 tracker alerts
b119174 chore(sdd): record B3 tracker alerts close-out
f679dd9 docs(superpowers): add Tier B B3 tracker alerts implementation plan
91e3193 chore(env): document Tier B B3 tracker alert env vars
...
e065ce7 feat(worker): add tracker alert and consent repository queries
147562c feat(worker): add tracker alerts migration 016

 .env.example                                       |    7 +
 .superpowers/sdd/progress.md                       |   16 +
 ...
 .../src/__tests__/tracker-alerts-migration.test.ts |   52 +
 .../src/__tests__/tracker-repository.test.ts       |  118 ++
 ...
 apps/worker/src/db/tracker-repository.ts           |  168 +-
 apps/worker/src/index.ts                           |    2 +
 apps/worker/src/processors/tracker-scan.ts         |   97 +-
 apps/worker/src/queue/tracker-scan-worker.ts       |   12 +-
 .../plans/2026-08-03-tier-b-b3-alerts.md           | 1897 ++++++++++++++++++++
 pnpm-lock.yaml                                     |    3 +
 29 files changed, 3100 insertions(+), 28 deletions(-)

diff --git a/.env.example b/.env.example
index 86905cf..3bbe98a 100644
--- a/.env.example
+++ b/.env.example
@@ -75,10 +75,17 @@ SEARXNG_BASE_URL=
...
index ed4b8fb..a99b3ed 100644
--- a/.superpowers/sdd/progress.md
+++ b/.superpowers/sdd/progress.md
@@ -155,10 +155,26 @@ Task 6: complete (commit 5aa87e5, review clean; full worker suite retains known
...
+import { checkIpRateLimit, createTrackerRepository, type TargetWithObservations, type AlertSummary } from "@seovista/worker";
 ...
 export type TrackerTargetActionState = {
   ...
   errors?: {
     ...
     keyword?: string[];
     domain?: string[];
@@ -19,30 +19,31 @@ export type TrackerTargetActionState = {
 };
...
@@ -61,21 +62,21 @@ export async function createTrackerTargetAction(
...
@@ -128,44 +129,64 @@ export async function createTrackerTargetAction(
     return {
       status: "error",
       errors: {
...
@@ -304,10 +325,33 @@ export async function createTrackerTargetForSessionAction(
     }
...
+  });
 ...
 export const TrackerSessionTargetSchema = z.object({
   keyword: z.string().trim().min(1, "Anahtar kelime gereklidir.").max(200, "Anahtar kelime 200 karakteri geçemez."),
   domain: z.string().trim().min(1, "Alan adı gereklidir.").max(253, "Alan adı 253 karakteri geçemez."),
diff --git a/apps/worker/migrations/016_create_tracker_alerts.sql b/apps/worker/migrations/016_create_tracker_alerts.sql
new file mode 100644
index 0000000..33e3105
--- /dev/null
+++ b/apps/worker/migrations/016_create_tracker_alerts.sql
@@ -0,0 +1,27 @@
...
+  });
+});
diff --git a/apps/worker/src/__tests__/tracker-alerts-migration.test.ts b/apps/worker/src/__tests__/tracker-alerts-migration.test.ts
new file mode 100644
index 0000000..97ca04c
--- /dev/null
+++ b/apps/worker/src/__tests__/tracker-alerts-migration.test.ts
@@ -0,0 +1,52 @@
...
index e0da7c3..293b205 100644
--- a/apps/worker/src/__tests__/tracker-repository.test.ts
+++ b/apps/worker/src/__tests__/tracker-repository.test.ts
@@ -134,11 +134,129 @@ describe("Tracker Repository", () => {
...
index c24d0ff..139445f 100644
--- a/apps/worker/src/db/tracker-repository.ts
+++ b/apps/worker/src/db/tracker-repository.ts
@@ -20,46 +20,87 @@ export interface TargetWithObservations {
...
@@ -157,19 +198,122 @@ export function createTrackerRepository(client: DbClient) {
...
diff --git a/docs/superpowers/plans/2026-08-03-tier-b-b3-alerts.md b/docs/superpowers/plans/2026-08-03-tier-b-b3-alerts.md
new file mode 100644
index 0000000..1f7f242
--- /dev/null
+++ b/docs/superpowers/plans/2026-08-03-tier-b-b3-alerts.md
@@ -0,0 +1,1897 @@
